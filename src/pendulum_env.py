...
class PendulumEnv(Env):
    ...
    def step(self, action: NDArray) -> tuple[NDArray, float, bool, bool, dict]:
        ...
        observation = self._create_observation_array(this_t - self.last_t, this_theta, this_pos_info)
    ...
    def _action_to_acceleration(self, action: float) -> int:
        mult = self.cfg.action_space.actual_half_range / (self.cfg.action_space.half_range ** 3)
    ...
    def _create_observation_array(self, delta_t: float, this_theta: float, this_pos_info: PosInfo) -> NDArray[np.int32]:
        ...
        return np.array([cos_theta, sin_theta, this_vel, this_motor_pos, this_norm_motor_vel], dtype=np.float32)
